...
const SubscribePage = () => {
  ...
  const [selectedPlan, setSelectedPlan] = useState<any>(initialPlan || null);
  ...
  const handleSuccess = async () => {
    ...
    try {
      ...
      // Redirect to MySubscriptions after 2.5s
      setTimeout(() => {
      ...
      <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {program.plans.map((plan: any) => (
          ...
          >
            ...
            <h3 className="text-xl font-bold uppercase text-center">{plan.name}</h3>
          ...
          {paymentOptions.map((method) => (
            ...
            >
              <div className="h-16 flex items-center justify-center">
                <img src={method.img} alt={method.name} className="max-h-14 w-auto object-contain" />
              </div>
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-[320px] text-center shadow-xl animate-scaleIn">
            ...
            <p className="text-gray-500 text-sm">Your subscription has been activated successfully.</p>
          </div>
        </div>
      )}
